...
final studyConfigurationProvider = StateNotifierProvider<StudyConfigurationNotifier, AsyncValue<StudyConfigurationSet>>((ref) {
  return StudyConfigurationNotifier(ref);
});

/// State notifier for managing study configuration
class StudyConfigurationNotifier extends StateNotifier<AsyncValue<StudyConfigurationSet>> {
  ...
  Future<void> updateLanguageConfig(AppLanguage language, LanguageStudyConfig config) async {
    ...
    try {
      ...
      // Update local state
      final updatedConfigSet = currentState.value.updateLanguageConfig(language, config);
...
final currentLanguageConfigProvider = Provider<LanguageStudyConfig?>((ref) {
  ...
  return configAsync.whenOrNull(
    data: (config) => config.currentConfiguration,
  );
}, dependencies: [studyConfigurationProvider]);

/// Provider for all enabled language configurations (cached)
final enabledLanguageConfigsProvider = Provider<List<LanguageStudyConfig>>((ref) {
  final configAsync = ref.watch(studyConfigurationProvider);
  final result = configAsync.whenOrNull(
    data: (config) => config.enabledConfigurations,
  ) ?? [];
...
final currentLanguageProvider = Provider<AppLanguage>((ref) {
  final configAsync = ref.watch(studyConfigurationProvider);
  return configAsync.whenOrNull(
    data: (config) => config.currentLanguage,
  ) ?? AppLanguage.latin;
}, dependencies: [studyConfigurationProvider]);
...
final isLanguageEnabledProvider = Provider.family<bool, AppLanguage>((ref, language) {
  final configAsync = ref.watch(studyConfigurationProvider);
  return configAsync.whenOrNull(
    data: (config) {
...
final languageConfigProvider = Provider.family<LanguageStudyConfig?, AppLanguage>((ref, language) {
  final configAsync = ref.watch(studyConfigurationProvider);
  return configAsync.whenOrNull(
