...
void main() {
  group('Provider Invalidation Tests', () {
    ...
    test(
      'word progress provider should refresh data after invalidation',
      () async {
        ...
        final wordProgressBefore = await container.read(
          wordProgressProvider.future,
        );
        ...
        final wordProgressCached = await container.read(
          wordProgressProvider.future,
        );
        expect(
          wordProgressCached.length,
          2,
          reason: 'Provider should still have cached data',
        );
        ...
        final wordProgressAfter = await container.read(
          wordProgressProvider.future,
        );
        expect(
          wordProgressAfter.isEmpty,
          isTrue,
          reason: 'Provider should be empty after invalidation',
        );
      },
    );
