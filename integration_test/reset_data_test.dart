...
void main() {
  ...
  group('Reset All Data Test', () {
    ...
    testWidgets('Reset All Data clears all user data completely', (
      tester,
    ) async {
      ...
      await dataService.saveQuizResult(
        'test_quiz_${DateTime.now().millisecondsSinceEpoch}',
        {
          'score': 4,
          'total': 5,
      ...
      debugPrint(
        '📝 Word progress entries before reset: ${wordProgressBefore.length}',
      );
      expect(
        wordProgressBefore.isNotEmpty,
        isTrue,
        reason: 'Should have word progress data',
      );

      // Check quiz results
      final quizResultsBefore = dataService.getQuizResults();
      debugPrint(
        '📊 Quiz results entries before reset: ${quizResultsBefore.length}',
      );
      expect(
        quizResultsBefore.isNotEmpty,
        isTrue,
        reason: 'Should have quiz results data',
      );

      // Check study sessions
      final studySessionsBefore = dataService.getStudySessions();
      debugPrint(
        '📈 Study sessions before reset: ${studySessionsBefore.length}',
      );
      expect(
        studySessionsBefore.isNotEmpty,
        isTrue,
        reason: 'Should have study sessions data',
      );
      ...
      Finder resetButton = find.bySemanticsLabel(
      ...
      if (resetButton.evaluate().isEmpty) {
        debugPrint(
          '❌ Reset My Data button not found in UI - checking available elements',
        );
      ...
      if (confirmButton.evaluate().isNotEmpty) {
        debugPrint(
          '✅ Confirmation dialog found using $buttonType - confirming deletion',
        );
        debugPrint(
          '📍 Button widget found: ${confirmButton.evaluate().first.widget}',
        );
        ...
        debugPrint(
          '⚠️ No confirmation dialog found - checking if reset happened directly',
        );
      }
      ...
      debugPrint(
        '📝 Word progress entries after reset: ${wordProgressAfter.length}',
      );
      expect(
        wordProgressAfter.isEmpty,
        isTrue,
        reason: 'Word progress should be empty after reset',
      );

      // Check quiz results are cleared
      final quizResultsAfter = dataServiceAfter.getQuizResults();
      debugPrint(
        '📊 Quiz results entries after reset: ${quizResultsAfter.length}',
      );
      expect(
        quizResultsAfter.isEmpty,
        isTrue,
        reason: 'Quiz results should be empty after reset',
      );
      ...
      expect(
        studySessionsAfter.isEmpty,
        isTrue,
        reason: 'Study sessions should be empty after reset',
      );
      ...
      expect(
        appSettingsAfter.isEmpty,
        isTrue,
        reason: 'App settings should be empty after reset',
      );
      ...
      final hasEmptyReviewMessage =
          find.textContaining('No words').evaluate().isNotEmpty ||
          find.textContaining('empty').evaluate().isNotEmpty;
      expect(
        hasEmptyReviewMessage,
        isTrue,
        reason: 'Review tab should show empty state after reset',
      );
      ...
      final hasNoQuizResults =
          find.textContaining('No quiz results').evaluate().isNotEmpty ||
          find.text('0').evaluate().isNotEmpty;
      expect(
        hasNoQuizResults,
        isTrue,
        reason: 'Progress tab should show no quiz results after reset',
      );

      debugPrint('✅ Reset All Data test completed successfully');
    });

    testWidgets('Reset All Data function works directly via LocalDataService', (
      tester,
    ) async {
      ...
      expect(
        dataService.getWordProgress().isEmpty,
        isTrue,
        reason: 'Word progress should be empty',
      );
      expect(
        dataService.getQuizResults().isEmpty,
        isTrue,
        reason: 'Quiz results should be empty',
      );
      expect(
        dataService.getStudySessions().isEmpty,
        isTrue,
        reason: 'Study sessions should be empty',
      );
      expect(
        dataService.getAppSettings().isEmpty,
        isTrue,
        reason: 'App settings should be empty',
      );
